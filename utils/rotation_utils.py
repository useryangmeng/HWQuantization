...
try:
    import torch_npu
    # 使用try-except检查NPU是否可用
    try:
        # 获取环境变量中指定的NPU设备，如果没有指定则使用默认设备
        npu_devices = os.environ.get('ASCEND_RT_VISIBLE_DEVICES', '6,7')
        # 使用第一个可用设备
        first_device = npu_devices.split(',')[0]
        DEV = torch.device(f'npu:{first_device}')
        # 尝试创建NPU张量来验证设备是否可用
        torch.randn(1).to(DEV)
    except:
    ...
    DEV = torch.device(f'cuda:{first_device}') if torch.cuda.is_available() else torch.device('cpu')
...
def fuse_ln_linear(layernorm: torch.nn.Module, linear_layers: typing.Iterable[torch.nn.Linear]) -> None:
    ...
    for linear in linear_layers:
        ...
        W_ = linear.weight.data.float()  # 使用float32代替double
        ...
        if hasattr(layernorm, 'bias'):
            ...
            linear.bias.data = linear.bias.data.float() + torch.matmul(W_, layernorm.bias.float())  # 使用float32代替double
...
def bake_mean_into_linear(linear: torch.nn.Linear) -> None:
    ...
    if linear.bias is not None:
        b_ = linear.bias.data.float()  # 使用float32代替double
...
def fuse_layer_norms(model):
    ...
    for W in model_utils.get_embeddings(**kwargs):
        W_ = W.weight.data.float()  # 使用float32代替double
...
def random_orthogonal_matrix(size, device):
    ...
    random_matrix = torch.randn(size, size, dtype=torch.float32).to(device)  # 使用NPU支持的float32
...
def random_block_orthogonal_matrix(size, device, block_size):
    ...
    Q_big = torch.zeros(size, size, dtype=torch.float32).to(device)  # 使用NPU支持的float32
...
def rotate_embeddings(model, Q: torch.Tensor) -> None:
    ...
    for W in model_utils.get_embeddings(model, model_type):
        ...
        W_ = W.weight.data.to(device=DEV, dtype=torch.float32)  # 使用NPU支持的float32
...
def rotate_attention_inputs(layer, Q, model_type) -> None:
    ...
    for W in [layer.self_attn.q_proj, layer.self_attn.k_proj, layer.self_attn.v_proj]:
        ...
        W_ = W.weight.to(device=DEV, dtype=torch.float32)  # 使用NPU支持的float32
        W.weight.data = torch.matmul(W_, Q).to(device="cpu", dtype=dtype)
...
def rotate_attention_output(layer, Q, model_type) -> None:
    ...
    if W.bias is not None:
        b = W.bias.data.to(device=DEV, dtype=torch.float32)  # 使用NPU支持的float32
...
def matmul_hadU_cuda_had(X, hadK, transpose=False):
    ...
    try:
        import torch_npu
        input = X.float().npu().view(-1, K, n // K)
    except ImportError:
...
def rotate_ov_proj(layer, Q, model_type, num_attention_heads, num_key_value_heads, head_dim):
    ...
    W_ = v_proj.weight.data.to(device=DEV, dtype=torch.float32)  # 使用NPU支持的float32
    shape = W_.shape
    ...
    if v_proj.bias is not None:
        B_ = v_proj.bias.data.to(device=DEV, dtype=torch.float32)  # 使用NPU支持的float32
        shape = B_.shape
    ...
    W_ = o_proj.weight.data.to(device=DEV, dtype=torch.float32)
    shape = W_.shape
