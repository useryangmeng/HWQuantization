...
from distill.layer_distill import multi_layer_distillation_loss, get_multi_layer_blocks
...
def distill_train(student_model, teacher_model, prefixed_key_values, args, trainloader, valloader, logger):
    logger.info("Starting distillation training (multi-layer)...")
    ...
    num_blocks = args.num_distill_blocks
    total_layers = len(layers_student)
    
    set_quant_state(student_model, weight_quant=True, act_quant=True)
    
    for start_layer in range(0, total_layers, num_blocks):
        end_layer = min(start_layer + num_blocks, total_layers)
        logger.info(f"=== Distilling blocks {start_layer} to {end_layer-1} ===")
        
        fp_blocks = [layers_teacher[i].to(dev) for i in range(start_layer, end_layer)]
        quant_blocks = [layers_student[i].to(dev) for i in range(start_layer, end_layer)]
        
        for param in teacher_model.parameters():
            param.requires_grad = False
        
        for fp_layer in fp_blocks:
            set_quant_state(fp_layer, weight_quant=False, act_quant=False)
        
        update_dataset(fp_blocks, quant_train_inps, fp_train_inps, dev, attention_mask, position_ids, prefixed_key_values)
        if quant_val_inps:
            update_dataset(fp_blocks, quant_val_inps, fp_val_inps, dev, attention_mask, position_ids, prefixed_key_values)
        
        for qlayer in quant_blocks:
        ...
        if args.quant_lr > 0:
            all_quant_params = []
            for qlayer in quant_blocks:
                all_quant_params.extend(list(quant_parameters(qlayer)))
            param.append({"params": all_quant_params, "lr": args.quant_lr})
        if args.weight_lr > 0:
            all_weight_params = []
            for qlayer in quant_blocks:
                all_weight_params.extend(list(weight_parameters(qlayer)))
        ...
        trainable_number = sum(trainable_parameters_num(qlayer) for qlayer in quant_blocks)
        ...
        for epoch in range(args.epochs):
            ...
            for idx in range(len(quant_train_inps)):
                with traincast():
                    ...
                    loss = multi_layer_distillation_loss(
                        fp_blocks, quant_blocks, input_data,
                ...
                for qlayer in quant_blocks:
                    all_trainable_params.extend(list(trainable_parameters(qlayer)))
            ...
            logger.info(f"Blocks {start_layer}-{end_layer-1} epoch {epoch} train_loss:{train_loss:.4f} val_loss:{val_loss:.4f} norm:{norm_mean:.8f} max_mem:{max_mem:.2f}MB time:{time.time()-start_time:.2f}s")
        ...
        update_dataset(quant_blocks, quant_train_inps, quant_train_inps, dev, attention_mask, position_ids, prefixed_key_values)
        if quant_val_inps:
            update_dataset(quant_blocks, quant_val_inps, quant_val_inps, dev, attention_mask, position_ids, prefixed_key_values)
        
        for i in range(start_layer, end_layer):
            layers_student[i] = layers_student[i].to("cpu")
            layers_teacher[i] = layers_teacher[i].to("cpu")
...
def main():
    ...
    parser.add_argument("--output_dir", default="./log/distill", type=str, help="Logging directory")
    parser.add_argument("--save_distill_dir", default=None, type=str, help="Directory for saving distilled model")
    
    parser.add_argument("--num_distill_blocks", type=int, default=2, help="Number of layers to distill together (2-4)")
    ...
    parser.add_argument("--attn_weight", type=float, default=0.5, help="Weight for attention KL divergence loss")
    ...
    student_model = distill_train(student_model, teacher_model, prefixed_key_values, args, trainloader, valloader, logger)
