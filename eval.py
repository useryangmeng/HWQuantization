...
from transformers import AutoTokenizer, AutoConfig, AutoModelForCausalLM, PreTrainedTokenizer, PreTrainedTokenizerFast
...
try:
    import torch_npu
    torch.backends.cudnn.benchmark = False  # NPU不需要这个设置
except ImportError:
...
def main():
    ...
    # 对于NPU，使用PyTorch标准随机种子设置即可，无需额外调用torch_npu.manual_seed
    # init logger
    if args.output_dir:
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.quant_model_path,legacy=False,trust_remote_code=True)
    ...
    try:
        import torch_npu
        # 获取用户指定的可见NPU设备
        visible_devices = os.environ.get('ASCEND_RT_VISIBLE_DEVICES', '0,1,2,3,4,5,6,7')
        visible_devices = list(map(int, visible_devices.split(',')))
        device_count = len(visible_devices)
        # 创建设备映射，只使用可见设备
        max_memory_dict = {i: args.max_memory for i in visible_devices}
        device_map = infer_auto_device_map(model, max_memory=max_memory_dict, no_split_module_classes=[block_class_name])
    except ImportError:
        device_count = torch.cuda.device_count()
        device_map = infer_auto_device_map(model, max_memory={i: args.max_memory for i in range(device_count)}, no_split_module_classes=[block_class_name])
