...
if not SKIP_CUDA_BUILD and CUDA_HOME is not None and torch.cuda.is_available():
...
def get_wheel_url():
    ...
    if torch_cuda_version is None:
        # CUDA is not available, use CPU-only version
        ...
        wheel_filename = f"{PACKAGE_NAME}-{fast_hadamard_transform_version}+cputorch{torch_version}cxx11abi{cxx11_abi}-{python_version}-{python_version}-{platform_name}.whl"
        wheel_url = BASE_WHEEL_URL.format(
            tag_name=f"v{fast_hadamard_transform_version}", wheel_name=wheel_filename
        )
        return wheel_url, wheel_filename
    
    torch_cuda_version = parse(torch_cuda_version)
    torch_version_raw = parse(torch.__version__)
