...
def update_dataset(layer, source_dataset, target_dataset, dev, attention_mask, position_ids, prefixed_key_values):
    # Use autocast that supports both CUDA and NPU
    try:
...
def block_ap(
    ...
    # Support both CUDA and NPU autocast
    try:
    ...
    # Clear cache - support both CUDA and NPU
    try:
    ...
    for block_index in range(len(layers)):
        ...
        if args.epochs > 0:
            ...
            for epoch in range(args.epochs):
                ...
                # Get max memory allocated - support both CUDA and NPU
            try:
                ...
                try:
                    ...
                    max_mem = 0
            logger.info(f"blocks {block_index} epoch {epoch} train_loss:{train_loss} val_loss:{val_loss}  norm:{gradient_norm:.8f} max memory_allocated {max_mem} time {time.time()-start_time} ")
